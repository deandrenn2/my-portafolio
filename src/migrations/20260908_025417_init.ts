...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  `)
  await db.run(sql`CREATE INDEX \`projects_photo_idx\` ON \`projects\` (\`photo_id\`);`)
  ...
  `)
  await db.run(sql`CREATE INDEX \`experience_video_idx\` ON \`experience\` (\`video_id\`);`)
  await db.run(sql`CREATE INDEX \`experience_photo_idx\` ON \`experience\` (\`photo_id\`);`)
  ...
  `)
  await db.run(sql`CREATE INDEX \`services_photo_idx\` ON \`services\` (\`photo_id\`);`)
  ...
  await db.run(sql`CREATE TABLE \`apps_tags\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`tag\` text,
  	FOREIGN KEY (\`_parent_id\`) REFERENCES \`apps\`(\`id\`) ON UPDATE no action ON DELETE cascade
  );
  `)
  await db.run(sql`CREATE INDEX \`apps_tags_order_idx\` ON \`apps_tags\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`apps_tags_parent_id_idx\` ON \`apps_tags\` (\`_parent_id\`);`)
  ...
  `)
  await db.run(sql`CREATE INDEX \`apps_photo_idx\` ON \`apps\` (\`photo_id\`);`)
  await db.run(sql`CREATE INDEX \`apps_updated_at_idx\` ON \`apps\` (\`updated_at\`);`)
  await db.run(sql`CREATE INDEX \`apps_created_at_idx\` ON \`apps\` (\`created_at\`);`)
  ...
  await db.run(sql`CREATE TABLE \`profiles_tags\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`tag\` text,
  	FOREIGN KEY (\`_parent_id\`) REFERENCES \`profiles\`(\`id\`) ON UPDATE no action ON DELETE cascade
  );
  `)
  await db.run(sql`CREATE INDEX \`profiles_tags_order_idx\` ON \`profiles_tags\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`profiles_tags_parent_id_idx\` ON \`profiles_tags\` (\`_parent_id\`);`)
  ...
  `)
  await db.run(sql`CREATE INDEX \`profiles_photo_idx\` ON \`profiles\` (\`photo_id\`);`)
}
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  await db.run(sql`DROP TABLE \`apps_tags\`;`)
  ...
  await db.run(sql`DROP TABLE \`profiles_tags\`;`)
